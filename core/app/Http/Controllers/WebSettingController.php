...
class WebSettingController extends Controller
{
    ...
    public function updateAchievement(Request $request)
    {
        ...
        if($request->hasFile('image')){
            $image = $request->file('image');
            $filename = 'achievement_background.jpg';
            $location = 'assets/images/' . $filename;
            Image::make($image)->resize(970,431)->save($location);
        }
    ...
    {
        $data['page_title'] = "Subscribe Section Background Image";
        return view('admin.webControl.subscribe',$data);
    }
    ...
    public function updateSubscribe(Request $request)
    {
        ...
        if($request->hasFile('image')){
            $image = $request->file('image');
            $filename = 'subscribe_background.jpg';
            $location = 'assets/images/' . $filename;
            Image::make($image)->resize(970,431)->save($location);
        }
        $notification = array('message' => ' Updated Successfully.', 'alert-type' => 'success');
